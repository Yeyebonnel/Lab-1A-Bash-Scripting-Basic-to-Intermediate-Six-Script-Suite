...
> This laboratory, we learned how to write bash scripts to do the things ITSU does routinely by hand. In each script we learned a new concept: variables and command substitution in Script 1, conditionals in Script 2, arrays and functions in Script 3, strict mode and traps in Script 4, parameter expansion in Script 5, and rsync backup in Script 6. The most difficult script to be 100 percent accurate with was Script 5 (log rotation), as we needed to ensure that exactly five versions were stored at any one time and the oldest version was removed each time. Also learned cron scheduling of scripts to run without supervision. We'd begin with log rotation if we were implementing this on ITSU's production server as another full day would be spent looking for log files that were lost or corrupted. It is also the most trivial task to automate as it doesn't interact with live data. Then, we would run the rsync backup and then the disk usage report. The cron schedule is important because it makes the scripts run even when the technicians are busy during enrolment. In this lab we have learned that automation isn't about coding it's about ensuring that our code will be reliable, it will log what it does, and that we can trust it to work without supervision.
